...
const skillDir = join(process.cwd(), '.claude/skills/huge');
const skillFile = join(skillDir, 'SKILL.md');

beforeAll(() => {
  mkdirSync(skillDir, { recursive: true });
  const big = 'token '.repeat(50000); // ~300k chars
  const md = `---\nname: huge\ndescription: A huge skill\nwhen_to_use: ${'use when token matters'}\naliases: [tok]\n---\n\n${big}\n`;
  writeFileSync(skillFile, md, 'utf-8');
});

afterAll(() => {
  try { rmSync(join(process.cwd(), '.claude/skills'), { recursive: true, force: true }); } catch {}
});
